<h2 style="text-align:center;">BY PRODUCT INVENTORY REPORT FOR THE MONTH OF <?php echo strtoupper(date("F", strtotime($mrdateis))) ?></h2>
<div class="card">
	...
	<div class="card-body">
		<table id="example4" class="table table-bordered" style="font-size:14px;">
			...
			<tbody>
				<?php 
				$sss = "SELECT * FROM `monthly_stock_report` AS ps inner join `product` as p WHERE ps.p_id=p.p_id AND `month_date_rep`='$mrdateis' GROUP BY  ps.p_id ";
					$gpd = $con->query($sss);
					$totalSP = 0;
					$totalCP = 0;
					$totalSk = 0;
					...
					while ($gpr = $gpd->fetch_array()) {
							...
							$newqtty = getTotalQuantityByProductMR($gpr['p_id']);
							$sr = getSpSupplier($gpr['dr_id'])->fetch_array();
							$tsp  = $gpr['p_qtty']*$gpr['p_price'];
							$tc = $newqtty*$gpr['p_cost'];
							$totalSP += $gpr['p_price'];
							$totalCP += $gpr['p_cost'];
							$totalSk += $newqtty;
							$totalNS += $tsp;
				 ...
				 <tr>
				 	...
				 	<td align="right" style="vertical-align:middle;" ><?php echo $newqtty ?></td>
